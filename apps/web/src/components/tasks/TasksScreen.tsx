...
export function TasksScreen() {
  ...
  async function remove(task: TaskCardData) { await deleteTask(task.id); await loadTasks(); }
  async function syncNotion(task: TaskCardData) { await syncTaskToNotion(task.id); await loadTasks(); }

  return (
    <AppShell title="Tasks" eyebrow="Move ideas forward">
      ...
      <GlassCard className="mb-6">
        <form onSubmit={submit} className="space-y-3">
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Task title" className="w-full rounded-[1.2rem] border border-white/10 bg-black/28 px-4 py-3 text-sm text-white outline-none placeholder:text-white/30" />
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Description" className="min-h-24 w-full rounded-[1.2rem] border border-white/10 bg-black/28 px-4 py-3 text-sm text-white outline-none placeholder:text-white/30" />
          <label className="flex items-center gap-2 text-sm text-white/58">
            <input type="checkbox" checked={syncToNotion} onChange={(e) => setSyncToNotion(e.target.checked)} className="accent-cyan-100" />
            Sync to Notion
          </label>
